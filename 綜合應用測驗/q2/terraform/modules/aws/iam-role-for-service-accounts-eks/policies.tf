...
data "aws_iam_policy_document" "ecr_rw" {
  count = var.create_role && var.attach_ecr_rw_policy ? 1 : 0
  statement {
    actions = [
...
resource "aws_iam_policy" "ecr_rw" {
  ...
  name_prefix = "${var.policy_name_prefix}ECR_RW_Policy-"
  path        = var.role_path
  ...
  policy      = data.aws_iam_policy_document.ecr_rw[0].json

  tags = var.tags
}

resource "aws_iam_role_policy_attachment" "ecr_rw" {
  count = var.create_role && var.attach_ecr_rw_policy ? 1 : 0

  role       = aws_iam_role.this[0].name
  policy_arn = aws_iam_policy.ecr_rw[0].arn
}
